		...
		for (const item of feed.items) {
			...
			if (type === 'post' || !type) {
				...
				if (rExcerpt.test(content)) {
					...
					content = md(content.substring(0, Math.min(1000, content.length)).trim()) + ' ' + postExcerpt + ' ' + md(content.substring(Math.min(1000, content.length), content.length).trim())
				}
			} else {
				content = md(content);
			}
	...
	if (posts.length >= 1) {
		for (const post of posts) {
			...
			try {
				...
				console.log(`About to replace for images`);

				post.content = post.content.replace('http://blog.davebrownphotography.com/wp-content/uploads', '/images/blog_images/uploads')
